...
export type UserQuery = Pick<User, "name" | "password">;
...
export default function(app: Application) {
    ...
    users.populate = function() {
        ...
        if (!authService || typeof authService.getStrategies !== "function") {
            console.error("Could not find an authentication service to hash password");
        }

        const [localStrategy] = authService.getStrategies("local") as LocalStrategy[];
        ...
        localStrategy.hashPassword("admin", {}).then(encoded =>
            User.create({
                ...
                .then((u: User) => {
                    console.log("+ user:", `${u.name}:admin#${u.id}`, "! Don't forget to remove it ASAP");
                })
                .catch(err => {
